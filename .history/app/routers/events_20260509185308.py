...
async def receive_event(event: RFIDEvent):
    ...
    try:
        ...
        event_dict["access_status"]=access_status
...
async def event_stream():
    # Real-time stream for Nuxt.js dashboard
    async def event_generator():
        last_index=0
        while True:
            if len(latest_events) > last_index:
                new_events=latest_events[last_index:]
...
Master, Above is my simulation.py code. Anything wrong?