...
def insert_detection_event(if not reader.get("is_active"):
    ...
    access_log_id = insert_access_log(
        ...
        geofence_id=None,
        decision="alert",
        reason=f"Inactive RFID reader received event: {reader_code}",
    )
    ...
    alert_id = insert_alert(
        ...
        severity="high",
        alert_type="INACTIVE_READER_EVENT",
        message=f"RFID event received from inactive reader '{reader_code}'.",
    )

    return {
        "ok": True,
        "decision": "alert",
        "message": "Inactive reader event detected",
        "detection_event_id": detection_event_id,
        "access_log_id": access_log_id,
        "alert_id": alert_id,
    }
...
def get_emergency_lockdown_status(organization_id: str | None) -> dict[str, Any]:
    ...
    with get_db_cursor() as cursor:
        cursor.execute(
            ...
            and setting_key = 'emergency_lockdown'
            limit 1
            """,
            (organization_id,),
        )
